...
在[Github/Kujiale-Mobile/Painter](https://github.com/Kujiale-Mobile/Painter)基础上封装，并修改优化了一些bug。灵活可自定义布局。

使用示例可以参考 [onlinePages/goodsdetail/index](http://gitlab.yonghuivip.com/web/yh-life-lite/blob/release/src/onlinePages/goodsdetail/index.js)。

**优势**

- 功能全，支持文本、图片、矩形、类型的 view 绘制
...
// page.json
{
  ...
  "usingComponents": {
    "c-share-card": "/components/business/ShareCard/index"
  }
}
...
// page.wxml
<view class='layout'>
  <c-share-card palette="{{shareCardJson}}" bind:onCardSaveCancel="saveCardCancel" bind:onCardSuccess="onShareCardSuccess" isShow="{{isShowShareCard}}" />
</view>
```
...
如你使用 wxss + wxml 规范进行绘制一样，组件需要根据一定的规范来进行图片绘制。当然组件的绘制规范要比 wxml 简单很多。
...
<details><summary>例子代码</summary><br>
...
align 类型与 css 中的 align 有些许不同。在 Painter 中 align 表示 view 本身的对齐方式，而不像 css 中表示对其子 view 的操作。align 可以作用在 Painter 支持的所有 view 上。它以设置的 left、top、right、bottom 的位置为基准，然后做不同的对齐操作。并且 align 在文字多行情况下，会影响多行文字的对齐方式。
...
1，目前组件中支持两种尺寸单位，px 和 rpx，代表的意思和小程序中一致，此处就不多说。
...
4，如果你只希望获得一张生成的图片来展示，可以把组件挪动到屏幕外进行绘制，绘制完后得到一张图片再进行展示，如下面这样。

```
<c-share-card style="position:fixed;top:-9999rpx" palette="{{userInfoTemplate}}" />
```
